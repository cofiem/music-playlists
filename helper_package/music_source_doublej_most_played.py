...
class MusicSourceDoubleJMostPlayed:

    def __init__(self, data_helper: DataHelper = None):
        self._logger = logging.getLogger('music_playlists.MusicSourceDoubleJMostPlayed')
        ...
        self._gmusic_playlist_id = os.getenv('MUSIC_SOURCE_DOUBLEJ_MOST_PLAYED_PLAYLIST_ID')
        self._playlist_name = 'Double J Most Played'
        self._group_id = 'doublej_most_played'
    ...
    def build_url(self, date_from, date_to, order='desc', limit='50', service='doublej'):
        qs = urlencode({
            'order': order,
            'limit': limit,
            'service': service,
